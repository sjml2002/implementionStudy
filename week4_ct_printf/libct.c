...
int ct_printf(const char* format, ...) {
    va_list args, ccopy, scopy, dcopy;
    va_start(args, format);

    int byteSize = (int)ct_strlen(format);
        ...
        if (format[i] == '%') {
            ...
            switch (format[i]) {
                ...
                case 's': {
                    ...
                    size_t len = ct_strlen(s);

                    if (write(1, s, len) != (ssize_t)len) return -1;
